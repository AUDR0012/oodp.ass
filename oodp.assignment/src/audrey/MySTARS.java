				...
				do
				{
					...
					{ // Add a Course
						...
						while (course.getGroups().size() < in_int1)
						{
							...
							{
								System.out.println("Group #" + course.getGroups().size() + 1 + " Session #" + group1.getSessions().size() + 1);
						...
						if (!Objects.equals(null, course = courseExist(courseList, in_string)))
						{
							do
							{
								...
								{ // Update Course Group
									do
									{
										in_int2 = Formatter.getIntegerInput(
												"\t\t1. Add Group" +
														"\t\t2. Edit Group\n" +
														"\t\t3. Delete Group\n" +
														"\t\t0. Back\n" +
										...
										{ // Edit Group
											do
											{
												...
												if ((choice1 = Formatter.withinRange("choice", 0, course.getGroups().size() + 1)) != 0)
												{
													group1 = course.getGroups().get(choice1 - 1);
													in_int3 = Formatter.getIntegerInput(
													...
													case 3:
													{ // Edit Session
														System.out.println("Current Session in Course Id: " + in_string.toUpperCase() + " Index Number: " + group1.getIndexNo());
														for (int i = 0; i < group1.getSessions().size(); i++)
														{
															session = group1.getSessions().get(i);
															System.out.println("\t\t\t" + i + 1 + ". " + session.getType() + " Venue: " + session.getVenue()
																	+ "Time: " + session.getDay() + session.getSTime() + session.getETime());
														}
														System.out.println("\t\t\t0. Back");
														if ((choice2 = Formatter.withinRange("choice", 0, group1.getSessions().size() + 1)) != 0)
														{
															if (Objects.equals(null, session = addSession(group1, choice2 - 1)))
															{
				...
				do
				{
					...
					{ // Add Course
						...
						if (!Objects.equals(null, group1 = groupExist(courseList, in_int1)))
						{
							...
							if (!courseAlreadyExist)
							{
								...
								{
									System.out.println(Formatter.tabs(40, "", "Index Number: " + group1.getIndexNo()) + "Course: " + course.getId());
					...
					{ // Drop Course
						if (student.getCourseGroups().size() > 0)
						{
							...
							if ((in_int1 = Formatter.withinRange("index number", 0, student.getCourseGroups().size() + 1)) > 0)
							{
								group1 = groupExist(courseList, student.getCourseGroups().get(in_int1 - 1).getIndexNo());
								course = group1.getCourse(courseList);

								System.out.println(Formatter.tabs(40, "", "Index Number: " + group1.getIndexNo()) + "Course: " + course.getId());
								course.printGroups(group1, null, PARSE_LENGTH, PARSE_DELIMITER);
								System.out.println(Formatter.tabs(50, "", "Course Type: " + course.getType()) + "Status: " + group1.findStudent(student, "status"));
													...
													for(Student tStudent : studentList)
													{
														if(tStudent.getMatricNo().equalsIgnoreCase(matricNo))
														{
															for(Group cg : tStudent.getCourseGroups())
															{
																...
																{
																	tStudent.getCourseGroups().remove(tStudent.getCourseGroups().indexOf(cg));
																	tStudent.getCourseGroups().add(group1);
																	tStudent.notifyMe(course.getId(), group1.getIndexNo(), notify, Group_Status.REGISTERED);
																	break;
																}
															}
														}
					...
					{ // Change Index Number of Group
						...
						if (!Objects.equals(null, group1 = groupExist(courseList, student.findGroup(in_int1).getIndexNo())))
						{
										...
										if (group2.getVacancy() > 0)
										{
											if ((overlaps = isOverlap(student, group2, group1)) == -1)
											{
												if (group2.getIndexNo() != group1.getIndexNo())
												{
													...
													System.out.println(Formatter.tabs(100, "", "Current Index Number: " + String.valueOf(group1.getIndexNo())) +
						...
						if (!Objects.equals(null, group1 = groupExist(courseList, student.findGroup(in_int1).getIndexNo())))
						{
							if (Enumerator.string(Group_Status.REGISTERED).equals(group1.findStudent(student, "status")))
							{
										...
										&& peer instanceof Student)
								{
									...
									if (!Objects.equals(null, group2 = groupExist(courseList, ((Student) peer).findGroup(in_int2).getIndexNo())))
									{
										if (Enumerator.string(Group_Status.REGISTERED).equals(group2.findStudent(((Student) peer), "status")))
										{
											if (Objects.equals((course = group1.getCourse(courseList)), group2.getCourse(courseList)))
											{
												if ((overlaps = isOverlap(student, group2, group1)) == -1
														&& (overlaps2 = isOverlap(((Student) peer), group1, group2)) == -1)
												{
													if (group1.getIndexNo() != group2.getIndexNo())
													{
														System.out.println("Subject: " + course.getId());
														System.out.println(Formatter.tabs(100, "", "Student #1 ") + "Student #2");
														System.out.println(Formatter.tabs(100, "", "Matric Number: " + student.getMatricNo() + " Index Number: " + group1.getIndexNo()) +
												...
												else
												{
													if (overlaps != -1)
													{
														System.out.println("Student " + student.getMatricNo() + "'s Course Index Number " + overlaps + " overlaps with " + in_int2 + ".");
													}
													else
													{
														System.out.println("Student " + ((Student) peer).getMatricNo() + "'s Course Index Number " + overlaps2 + " overlaps with " + in_int1 + ".");
													}
												}
											}
											else
											{
												System.out.println("Index Number " + in_int1 + " and Index Number " + in_int2 + " exist in a different Course.");
											}
										}
										else
										{
